...
const placeholderEvents: VenueEvent[] = [
  {
    id: 'e_sigma-chi',
    ...
    async function loadEvents() {
      ...
      try {
        const { data, error } = await supabase
          .from('venues_events')
          .select('id,title,host_name,venue_type,ticket_price_ga,ticket_price_vip,theme_tag,date_time')
          .order('date_time', { ascending: true })

        if (error) {
          // Don't surface development overlay; warn and fallback to mock data
          console.warn('Could not load venue events from Supabase:', error)
      ...
      <div className="no-scrollbar h-full overflow-y-auto pb-32">
        <div className="px-4 pt-5">
          ...
          <p className="mt-2 max-w-xl text-sm leading-6 text-muted-foreground">
            Browse premium High Point events and reserve your spot with a polished checkout sheet.
          </p>
        </div>
